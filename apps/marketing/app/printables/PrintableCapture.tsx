...
export default function PrintableCapture({ slug }: { slug: string }) {
  ...
  return (
    <div className="print-hide fixed inset-x-4 bottom-4 z-50 mx-auto max-w-sm rounded-card border border-border bg-card-bg p-5 shadow-[0_12px_40px_rgba(81,0,0,0.18)] sm:left-auto sm:right-6 sm:mx-0">
      <button
        type="button"
        onClick={dismiss}
        aria-label="Dismiss"
      ...
      {status === "done" ? (
        <p className="pr-6 text-base font-medium text-foreground">You&apos;re in. Talk soon.</p>
      ) : (
        <>
          <p className="pr-6 text-base font-bold text-foreground">Hope that helped.</p>
